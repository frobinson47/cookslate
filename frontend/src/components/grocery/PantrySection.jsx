  ...
  return (
    <div className="bg-surface rounded-2xl shadow-md overflow-hidden">
      <div className="px-4 py-3 border-b border-cream-dark flex items-center gap-2">
        ...
        <h2 className="font-semibold text-brown">Household Pantry</h2>
      ...
      {items.length > 0 && (
        <div className="px-4 pb-2 flex flex-wrap gap-2">
          {items.map(item => {
            const canEdit = item.is_owner || isAdmin;
            return (
              <span
                key={item.id}
                title={!item.is_owner && item.added_by_username ? `Added by ${item.added_by_username}` : undefined}
