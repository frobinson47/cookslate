...
export default function PantryScanUploadForm({ onScanSuccess, isLoading, onScan, keyConfigured }) {
  ...
  if (!keyConfigured) {
    return (
      <div className="bg-surface rounded-2xl shadow-md p-6 text-center">
        <ScanLine size={20} className="text-terracotta mx-auto mb-2" />
        <p className="text-sm text-brown-light mb-3">
          Connect your OpenAI API key in Settings to scan your pantry.
        </p>
  ...
  return (
    <div className="bg-surface rounded-2xl shadow-md p-6">
      ...
      <p className="mt-3 text-xs text-warm-gray text-center">
        Works best with a well-lit photo of one shelf at a time.
      </p>
    </div>
  );
}
