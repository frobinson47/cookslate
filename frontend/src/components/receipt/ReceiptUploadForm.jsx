...
export default function ReceiptUploadForm({ onImportSuccess, isLoading, onImport, keyConfigured }) {
  ...
  if (!keyConfigured) {
    return (
      <div className="bg-surface rounded-2xl shadow-md p-6 text-center">
        <Receipt size={20} className="text-terracotta mx-auto mb-2" />
        <p className="text-sm text-brown-light mb-3">
          Connect your OpenAI API key in Settings to scan receipts.
        </p>
    ...
    <div className="bg-surface rounded-2xl shadow-md p-6">
      ...
      {isLoading && (
        <div className="flex items-center gap-2 mt-3 text-sm text-warm-gray">
          <Spinner size="sm" />
          Reading line items from receipt...
        </div>
      )}

      <p className="mt-3 text-xs text-warm-gray text-center">
        Works best with a flat, well-lit photo of the full receipt.
      </p>
    </div>
  );
}
